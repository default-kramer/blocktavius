...
namespace Blocktavius.Tests
{
	...
	public sealed class Test1
	{
		...
		[TestMethod]
		[Timeout(1000 * 60)]
		public void ExerciseJauntShift()
		{
			var prng = PRNG.Create(new Random());
			Console.WriteLine(prng.Serialize());
			...
			int minRunLength = jauntSettings.RunLengthProvider.MinValue;
			int maxRunLength = jauntSettings.RunLengthProvider.MaxValue;
			...
			while (runs-- > 0)
			{
				var initial = Jaunt.Create(prng, jauntSettings);
				var prev = initial;

				int shifts = shiftsPerRun;
				while (shifts-- > 0)
				{
					var current = prev.Shift(prng, minRunLength, maxRunLength);

					// Verify basic properties
					Assert.AreEqual(prev.NumRuns, current.NumRuns);
					Assert.AreEqual(prev.TotalLength, current.TotalLength);

					for (int i = 0; i < current.Runs.Count; i++)
					{
						totalRuns++;
						var currentRun = current.Runs[i];

						Assert.IsTrue(currentRun.length >= minRunLength, $"Run {i} length {currentRun.length} < min {minRunLength}");
						...
						// Check if this run is unchanged from previous
						var prevRun = prev.Runs[i];
						if (currentRun.start == prevRun.start && currentRun.length == prevRun.length)
						{
							unchangedRuns++;
						}
					}
			...
			// Statistical checks similar to ExerciseCornerShifter
			if (longRuns > totalRuns / 20)
			{
				Assert.Fail($"Too many long runs: {longRuns} / {totalRuns}");
			}
			if (unchangedRuns > totalRuns / 10)
			{
				Assert.Fail($"Too many unchanged runs: {unchangedRuns} / {totalRuns}");
			}
			if (unchangedRuns < totalRuns / 1000)
			{
